...
export const getCosts = async (userId: string, filters: {
  type?: string;
  fuelType?: string;
...
export const updateCost = async (userId: string, id: string, data: Partial<IEnergyCost>) => {
  ...
  if (data.costCalculationMethod === 'auto' && data.type !== 'solar') {
    ...
    const existing = await EnergyCost.findOne({ _id: id, userId });
    if (existing) {
  ...
  return EnergyCost.findOneAndUpdate(
    { _id: id, userId },
    data,
    { new: true }
  );
...
export const getCostSummary = async (userId: string) => {
  return EnergyCost.aggregate([
    { $match: { userId: new EnergyCost.base.Types.ObjectId(userId) } },
    {
      $group: {
        _id: '$type',
        totalCost: { $sum: '$cost' },
